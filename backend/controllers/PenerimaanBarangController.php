...
class PenerimaanBarangController extends Controller
{
     ...
     * @return mixed
     */
   public function actionShowPenerimaan($bulan,$tahun){
		$searchModel = new PenerimaanBarangSearch();
		$where = ['YEAR(tgl_faktur)'=>$tahun];
		$andWhere = ['MONTH(tgl_faktur)'=>$bulan];
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams,$where,$andWhere);
		return $this->renderAjax('show-penerimaan',[
    ...
    public function actionIndexPenerimaan(){
		return $this->render('index-penerimaan');
	}
	...
	public function actionCetak($bulan,$tahun){
		$model = PenerimaanBarang::find()->where(['YEAR(tgl_faktur)'=>$tahun])->andwhere(['MONTH(tgl_faktur)'=>$bulan])->all();
	...
	public function actionCetakData($bulan,$tahun){
		$model = PenerimaanBarang::find()->where(['YEAR(tgl_faktur)'=>$tahun])->andwhere(['MONTH(tgl_faktur)'=>$bulan])->all();
		$content = $this->renderPartial('cetak-data',['model' => $model]);
		$pdf = new Pdf([
		   'mode' => Pdf::MODE_CORE,
		   'destination' => Pdf::DEST_BROWSER,
		   'format' => Pdf::FORMAT_A4, 
    ...
    public function actionPrintRekap($start,$end) {
	  ...
	  return $pdf->render(); 
		...
		$content = file_get_contents($url);
		$json = json_decode($content, true);
		$content = $this->renderPartial('rekap-atk',['model' => $json,'start'=>$start,'judul'=>'Barang & ATK']);
	...
	public function actionSelesai($id){
		...
		foreach($penerimaan as $p){
			$obat_bacth = Obat::find()->where(['id'=>$p->idbarang])->one();
    ...
    public function actionPenerimaanSelesai($id){
      ...
      public function actionView($id){
		...
		if($barang->load(Yii::$app->request->post())) {
			...
			$dataBarang->stok_apotek = 0;
			$dataBarang->stok_gudang = 0;
			$dataBarang->harga_jual = $dataBarang->harga_beli + ($dataBarang->harga_beli*0.25);
			if($dataBarang->save()){
