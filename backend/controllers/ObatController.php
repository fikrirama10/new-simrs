...
class ObatController extends Controller
{
    ...
    public function actionIndexBacth(){
		$searchModel = new ObatBacthSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        return $this->render('index-bacth', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
	}
    public function actionHapusBacth($id){
		$model = ObatBacth::findOne($id);
		$permintaan = PermintaanObatRequest::find()->where(['idbacth'=>$model->id])->count();
		if($permintaan > 0){
			Yii::$app->session->setFlash('warning', 'Obat Sudah ada di trx');
	...
	public function actionShowKartu2($id,$idbacth,$awal,$akhir,$asal,$jenis){
		if($idbacth == 0){
			if($jenis == 0){
    ...
    public function actionEditBacth($id){
		...
		if ($model->load(Yii::$app->request->post())) {
			...
			$model->harga_jual = $model->harga_beli + ($model->harga_beli * 0.25);
			if($model->save()){				
    ...
    public function actionView2($id)
    {
		...
		if ($obat->load(Yii::$app->request->post())) {
			$obat->harga_jual = $obat->harga_beli + ($obat->harga_beli * 0.25);
     ...
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionShowGrafik($idobat,$bulan,$tahun){
		$obat = Obat::findOne($idobat);
		$url = Yii::$app->params['baseUrl'].'dashboard/obat-rest/penggunaan-obat?idgudang='.Yii::$app->user->identity->userdetail->idgudang.'&idobat='.$idobat.'&bulan='.$bulan.'&tahun='.$tahun;
		$content = file_get_contents($url);
		$json = json_decode($content, true);
		...
		return $this->renderAjax('show-grafik',[
			'model'=>$json,
			'obat'=>$obat->nama_obat,
			'bulan'=>$bulann,
			'bulann'=>$bulan,
	...
	public function actionShowData($idobat,$awal,$akhir){
		$url = Yii::$app->params['baseUrl'].'dashboard/obat-rest/obat-mutasi?idgudang='.Yii::$app->user->identity->userdetail->idgudang.'&idobat='.$idobat.'&start='.$awal.'-01&end='.$akhir;
		$content = file_get_contents($url);
		$json = json_decode($content, true);
		return $this->renderAjax('_formMutasi',[
			'json'=>$json
		]);
	}
	public function actionShowKartu($idobat,$awal,$akhir){
		$url = Yii::$app->params['baseUrl'].'dashboard/obat-rest/kartu-stok?idgudang='.Yii::$app->user->identity->userdetail->idgudang.'&idobat='.$idobat.'&start='.$awal.'-01&end='.$akhir;
		$content = file_get_contents($url);
		$json = json_decode($content, true);
		return $this->renderAjax('_formKartu',[
			'json'=>$json
		]);
	}
		...
		if ($model->load(Yii::$app->request->post())) {
			...
			if(Yii::$app->user->identity->userdetail->idgudang == 1){
				if($koreksi == null){
					...
					$jumlah = $model->stok_gudang - $koreksi;
					Yii::$app->kazo->kartuStok($model->id,0,1,$jumlah,1);	
					Yii::$app->kazo->mutasiStok($model->id,0,5,8,$jumlah,0,$model->stok_gudang,1);
					Yii::$app->kazo->gudangStok(Yii::$app->user->identity->userdetail->idgudang,$model->id,$jumlah,$tgl,1);
					$model->stok_gudang = $koreksi;
				}else if($koreksi > $model->stok_gudang){
					$jumlah = $koreksi - $model->stok_gudang;
					Yii::$app->kazo->kartuStok($model->id,0,1,$jumlah,2);	
					Yii::$app->kazo->mutasiStok($model->id,0,3,7,$jumlah,0,$model->stok_gudang,1);
					$model->stok_gudang = $koreksi;
					Yii::$app->kazo->gudangStok(Yii::$app->user->identity->userdetail->idgudang,$model->id,$jumlah,$tgl,2);
				}
			}else if(Yii::$app->user->identity->userdetail->idgudang == 2){
				if($koreksi == null){
					$model->stok_apotek = $model->stok_apotek;
				}else if($koreksi < $model->stok_apotek){
					$jumlah = $model->stok_apotek - $koreksi;
					Yii::$app->kazo->kartuStok($model->id,0,2,$jumlah,2);	
					Yii::$app->kazo->mutasiStok($model->id,0,5,8,$jumlah,0,$model->stok_apotek,2);
					Yii::$app->kazo->gudangStok(Yii::$app->user->identity->userdetail->idgudang,$model->id,$jumlah,$tgl,2);
					$model->stok_apotek = $koreksi;
				}else if($koreksi > $model->stok_apotek){
					$jumlah = $koreksi - $model->stok_apotek;
					Yii::$app->kazo->kartuStok($model->id,0,2,$jumlah,2);	
					Yii::$app->kazo->mutasiStok($model->id,0,3,7,$jumlah,0,$model->stok_apotek,2);
					$model->stok_apotek = $koreksi;
					Yii::$app->kazo->gudangStok(Yii::$app->user->identity->userdetail->idgudang,$model->id,$jumlah,$tgl,2);
				}
			}
			if($model->save()){				
				return $this->redirect(['/obat/view?id='.$model->id]);				
			}
		}
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        $detail = ObatBacth::find()->where(['idobat'=>$model->id])->all();
        foreach ($detail as $d){
            $d->delete();
        }
        $model->delete();
