...
<div class='row'>		
	...
	<div class='col-md-9'>
		<div class="box">
			...
			<div class="box-body">
				<?php 
					if($model->status == 1){
						$tombol =  Html::a('<i class="fas fa-redo"></i> Masuk Stok', ['/gudang/masuk-stok-atk?id='.$model->id], ['class' => 'btn bg-navy','id'=>'confirm']);
					}else{
						$tombol ='';
					}
				?>
				...
				'columns' => [
					...
					[
						...
						'value' => function ($model, $key, $index) { 
								$penerimaan = BarangPenerimaan::find()->where(['id'=>$model->idpenerimaan])->one();
								if($penerimaan->status == 1){
									if($model->diterima != 1){
										return "<input class='form-control' readonly id='jumlahL-".$model->id."' type='text' value='".$model->qty."'>";
									}else{
										return "<input class='form-control' readonly id='jumlahL-".$model->id."' type='text' value='".$model->qty_diterima."'>";
									}
								}else{
									return $model->qty_diterima;
								}
						},					
	...
	$urlEdit = Url::to(['gudang/terima-atk']);
	$this->registerJs("
