		...
		<div class='col-md-6'>
			...
			<div class='box box-body'>
				<table class='table table-bordered'>
					...
					<?php $no=1; foreach($detail as $pl){ ?>
					<tr>
						<td><?= $no++ ?></td>
						<td><?= $pl->barang->nama_barang ?></td>
						<td><?= $pl->qty ?></td>
						<td>
							<?php if($pl->status == 3){ ?>
						...
						<td>
						<?php if($pl->status == 3){ ?>
							<a href='<?= Url::to(['gudang/amprah-berikan?id='.$pl->id]) ?>' class='btn btn-xs btn-success'>Berikan</a> 
							<a  data-toggle='modal' data-target='#mdPermintaan<?= $pl->id ?>' class='btn btn-xs btn-primary'>Lihat Stok</a>
						<?php }else{ ?>
							<a href='<?= Url::to(['gudang/amprah-koreksi?id='.$pl->id]) ?>' class='btn btn-xs btn-success'>Koreksi</a> 
						<?php } ?>
						</td>
					</tr>
					<?php 
					$urlEdit = Url::to(['gudang/edit-item-amprah']);
					$this->registerJs("
				...
				<?php if($model->status == 4){ ?>
					<a href='<?= Url::to(['gudang/amprah-selesai?id='.$model->id])?>' class='btn btn-primary' id='confirm-berikan'>Selesai</a>
				<?php } ?>
